...
export interface RenderRouteOptions {
  ...
  loaderData?: unknown
  /**
   * Route context to provide in place of the route's `beforeLoad` result,
   * e.g. `{ user }` for the admin layout.
   */
  context?: Record<string, unknown>
  /** Wraps the router, e.g. a `ToastProvider` the real layout would mount. */
 ...
 * route's `beforeLoad` guard (or with one that returns `context`). Mock the route's server-function imports with
 * `vi.mock` first. The router mounts asynchronously: query with `findBy*`.
 */
export function renderRoute(
  route: AnyRoute,
  { id, path, url, loaderData, context, wrapper }: RenderRouteOptions,
) {
  ...
  const pageRoute = route.update({
    ...
    beforeLoad: context ? () => context : undefined,
    loader: () => loaderData,
